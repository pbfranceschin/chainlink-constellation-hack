...
describe.skip("Pool scenarios test", function () {
    ...
    const printParams = async(player: number, account: string, pick:number) => {
        ...
        // const yieldWithdrawn = await pool.yieldWithdrawn();
        console.log('current Yield:', yield_ );
        ...
        console.log(`\n==PLAYER ${player}==`)
        console.log('total shares:', await vApi.totalSupply());
        let outcomeSh = await pool.getSharesByOutcome(pick);
        console.log(`shares by outcome#${pick}:`, outcomeSh);
        const outcomSh2Ass = await vApi.convertToAssets(outcomeSh); 
        console.log(`outcome#${pick} sharesToAss: ${outcomSh2Ass}`);
        const outcomStake = await pool.getStakeByOutcome(pick);
        console.log(`outcome#${pick} stake: ${outcomStake}`)
        let outComYield = outcomSh2Ass - outcomStake;
